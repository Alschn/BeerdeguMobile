// inspired by from `rest_framework/status.py`
const between = (value: number, min: number, max: number) =>
  !!value && value >= min && value < max;

export const isInformationalCode = (status: number): boolean =>
  between(status, 100, 200);
export const isSuccessCode = (status: number): boolean =>
  between(status, 200, 300);
export const isRedirectCode = (status: number): boolean =>
  between(status, 300, 400);
export const isClientErrorCode = (status: number): boolean =>
  between(status, 400, 500);
export const isServerErrorCode = (status: number): boolean =>
  between(status, 500, 600);
